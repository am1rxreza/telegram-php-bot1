...
/*
Developer : @Cristianoronal_d_o
Channels : @SourceCrystal, @botbiso
*/

$botToken = '7892797628:AAGg8zAP_MHmi0b_d4JsofhQgVXBFhNbyC0';
$targetChannel = 'p1ay7';
$sourceChannels = array_unique([
    '@NPROXY',
    '@ConfigsHubPlus',
    '@ConfigTech1',
    '@e2vpn',
...
// حالت وارد کردن تعداد
if ($state === 'waitingForCount') {
    if (is_numeric($text)) {
        $count = min(max((int)$text, 1), 1000);
        sendMessage($chat_id, "⏳ در حال ارسال $count کانفیگ ، لطفا صبر کنید...");
        ...
        foreach ($allConfigs as $config) {
            ...
            usleep(250000); // تاخیر برای جلوگیری از spam
        }

        sendMessage($chat_id, "✅ ارسال کانفیگ‌ها تمام شد!\nتعداد کانفیگ‌های ارسال شده: " . count($allConfigs));
        setState($chat_id, 'idle');
        exit;
    } else {
...
// حالت شروع ارسال کانفیگ
if (mb_strtolower($text) === 'ارسال کانفیگ‌') {
    ...
    sendMessage($chat_id, "لطفا تعداد کانفیگ‌هایی که می‌خواهید دریافت کنید را وارد کنید (بین 1 تا 10):", $cancelKeyboard);
    exit;
}
